...
Evaluation inbox entries, primary form sections, and profile summary panels SHALL be rendered as visually distinct cards: rounded corners (`rounded-xl`), subtle border, soft shadow, and a raised surface color—white in Light mode and dark slate in Midnight mode consistent with `#131B2B` (or semantic equivalent).
...
- **THEN** that block SHALL match the elevated card treatment
